...
**Status as of 2026-08-22: 814 / 1,000 monthly API pulls used** across all sites/projects on this account that use Scrape.do (currently: `housenomics/mls-proxy.php`, `homeward/api/mls-proxy.php`, and the shared `backend/property-lookup.php` that supersedes both — see the "Shared Backend Service" section below). The token is shared across every project that reads `SCRAPE_DO_TOKEN` from `/home/nyctltlc/api.env`, so a call from ANY of them draws down the same pool.
...
| **`greener-grass`** | `/greener-grass/` (titled "Greener Grass") | Real-time relocation analytics dashboard — climate, transit radius scoring, geocoding. | Vanilla JS, Open-Meteo API, Leaflet JS, Nominatim geocoding. |
...
| **`housenomics`** | `/housenomics/` (titled "Housenomics") | Mortgage/interest calculators, amortization schedule, MLS listing scraper. | Vanilla JS + its own PHP proxies (`mls-proxy.php`, `rates-proxy.php`) for public rate/listing data only. Calculator inputs are localStorage-only — see `storage.js` header comment; do NOT add a server sync for them. |
| **`sunset-clause`** | `/sunset-clause/` (titled "Sunset Clause") | Long-term retirement savings projection tool with charts/KPIs. | Vanilla JS (chart.js-style modules: `charts.js`, `kpi.js`, `simulation.js`). No network calls; fully client-side. |
| **`certforge`** | `/certforge/` (titled "Certforge") | CISM security certification exam prep — quiz simulator, flashcards, bookmarks, attempt tracking. | Vanilla JS front end. All progress/bookmarks/attempts are localStorage-only; the question/flashcard seed is loaded from a static `backend/cism_seed.json` file, not an API. |
...
| **`fitstack`** | `/fitstack/` (titled "Fitstack") | Hardware-aware AI discovery engine — recommends Hugging Face models that fit a user's VRAM/RAM without OOM errors. | Python **FastAPI** backend (`backend/main.py`, `hardware.py`, `hf_client.py`, `engine.py`) + vanilla JS front end. Self-contained, separate from the shared Node backend. Its `deploy/passenger_wsgi.py` mounts the API at a hardcoded `MOUNT_PREFIX` that must match cPanel's "Setup Python App" Application URL exactly — a server-side control-panel setting, not something version-controlled here. |
...
| **`dibs`** | `/dibs/` (titled "Dibs" — private, login-gated — excluded from `robots.txt`/sitemap, unlike the public tools above) | MLS & Redfin property-hunting dashboard for Josh's own house search — scrapes Matrix MLS portal listings via a bookmarklet (with a "deep scrape" mode for full detail + photo galleries), enriches with Redfin data, supports favorites/ratings/shared notes, a comparison matrix, map view, and a deterministic "Top Picks" recommendation scorer. Has its own detailed `claude/` plan docs in its Claude Project. | PHP + SQLite (`data/properties.db`), vanilla JS ES modules, Leaflet JS. |
...
2. **Security & Content Security Policy**:
   ...
   - Prevent direct public access to sensitive files (`.db`, `.log`, `.env`, `.py`, `.sh`) and version control directories (`.git`, `.svn`, `.hg`) via `.htaccess`. Never hardcode API keys/tokens in committed source — read them from `/home/nyctltlc/api.env` via `getenv()`/`cfgEnv()` per the existing convention in `nyctos-gig-grid/config.php` and `housenomics/mls-proxy.php`.
